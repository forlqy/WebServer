...
#include <limits.h>
#include <netinet/tcp.h> //tcp发包缓存(效率)TCP_NODELAY

namespace sylar{

static sylar::Logger::ptr g_logger = SYLAR_LOG_NAME("system"); //log对象若不是static则为全局范围，会出现链接报错
...
Socket::Socket(int family, int type, int protocol) : m_sock(-1), m_family(family), m_type(type), m_protocol(protocol), m_isConnected(false) {
}
...
int64_t Socket::getSendTimeout(){
    ...
    if(ctx){
        return ctx->getTimeout(SO_SNDTIMEO);//SO_SNDTIMEO是一个套接字选项，它用于设置在发送数据时等待对端响应的超时时间。具体来说，它指定了当调用send()发送数据时，如果在指定的时间内没有收到对端的响应，则send()操作将返回一个错误，并且应用程序可以根据返回的错误代码来采取相应的操作。
    }
    return -1;
}
...
Socket::ptr Socket::accept(){
    ...
    int newsock = ::accept(m_sock, nullptr, nullptr); //::代表用的是全局的accept而不是自定义的
    if(newsock == -1){
