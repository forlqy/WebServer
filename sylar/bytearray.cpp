...
//在二进制中，负数一般有很多1，此时进行转换可降低压缩是需要压缩的字节数，（-1在二进制所有位均为1，压缩它需要5字节）
...
/*
Zigzag编码是一种将有符号整数转换为无符号整数的编码方式，用于在变长编码中更有效地表示负数。该编码方法通过将负数的位值映射到奇数位，正数的位值映射到偶数位，从而实现编码的压缩。
*/
...
void ByteArray::writeUint32(uint32_t value){
    ...
    uint8_t i = 0;
    while(value >= 0x80){
/*在循环中，将 `value` 的低7位与0x7F进行按位与运算，并将结果与0x80进行按位或运算，得到一个7位的编码片段。这样做是为了将 `value` 拆分为多个7位的编码片段，每个片段的最高位（第8位）都设置为1，表示还有后续片段。
将上一步得到的编码片段存储到临时数组 `tmp` 中，并将 `value` 右移7位，继续处理剩余的片段。*/
...
void ByteArray::write(const void* buf, size_t size){
    ...
    size_t npos = m_position % m_baseSize; //当前分区当前节点的位置
