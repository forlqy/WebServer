...
//怎么在main函数之前执行一段程序
/*
全局变量在进入main函数之前就会初始化
这时运用单例模式声明一个静态全局变量
在进入main之前就会对struct进行构造
在构造函数里进行init即可
*/
...
//比起协程 hook实现异步才是高并发真正的精髓
//下面是实现异步的精华
//这里hook的高并发是io任何的操作只要没有ready就添加到event等到ready了继续执行并且添加事件超时(do_io)
retry:
...
//加入定时器后，等待条件满足，唤醒定时器，执行后续步骤（成功的将同步操作达到了异步效果）
        if(to != (uint64_t)-1){
...
int fcntl(int fd, int cmd, ... /* arg */ ) { //获取NONBLOCK信息
    va_list va;
    va_start(va, cmd);
    switch(cmd) {
        case F_SETFL:
