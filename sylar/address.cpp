...
template<class T>
static uint32_t CountBytes(T value) { //统计一个int有多少个1(用&效率最高)
    uint32_t result = 0;
    for(; value; ++result) {
        value &= value - 1;
    }
...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,
            int family, int type, int protocol){ //用于将主机名解析成IP地址
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port){
    ...
    int result = inet_pton(AF_INET, address, &rt->m_addr.sin_addr); //将IP地址的文本表示转换为二进制形式的网络字节序数值
    if(result <= 0){
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port){
    ...
    int result = inet_pton(AF_INET6, address, &rt->m_addr.sin6_addr); //将IP地址的文本表示转换为二进制形式的网络字节序数值
    if(result <= 0){
