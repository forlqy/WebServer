...
HttpResult::ptr HttpConnection::DoGet(const std::string& url, uint64_t timeout_ms, const std::map<std::string, std::string>& headers, const std::string& body){
    ...
    if(!uri){
        return std::make_shared<HttpResult>(static_cast<int>(HttpResult::Error::INVALID_URL), nullptr, "invalid url: " + url); //enum class不支持隐式转换成int型，enum支持
    }
...
HttpResult::ptr HttpConnection::DoPost(const std::string& url, uint64_t timeout_ms, const std::map<std::string, std::string>& headers, const std::string& body){
    Uri::ptr uri = Uri::Create(url);
    if(!uri){
        return std::make_shared<HttpResult>(static_cast<int>(HttpResult::Error::INVALID_URL), nullptr, "invalid url: " + url);
    }
...
HttpResult::ptr HttpConnection::DoRequest(HttpMethod method, Uri::ptr uri, uint64_t timeout_ms, const std::map<std::string, std::string>& headers, const std::string& body){
    ...
    bool has_host = false;
    for(auto& i : headers){
        if(strcasecmp(i.first.c_str(), "connection") == 0){ //不关心大小写
...
HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req, Uri::ptr uri, uint64_t timeout_ms){
    ...
    if(!addr){
        return std::make_shared<HttpResult>(static_cast<int>(HttpResult::Error::INVALID_HOST), nullptr, "invalid host: " + uri->getHost());
    }
    Socket::ptr sock = Socket::CreateTCP(addr);
    if(!sock){
        return std::make_shared<HttpResult>(static_cast<int>(HttpResult::Error::CREATE_SOCKET_ERROR), nullptr, "create socket fail:" + addr->toString() + std::to_string(errno) + " errstr=" + std::string(strerror(errno)));
    }
    if(!sock->connect(addr)){
        return std::make_shared<HttpResult>(static_cast<int>(HttpResult::Error::CONNECT_FAIL), nullptr, "connect fail: " + addr->toString());
    }
    sock->setRecvTimeout(timeout_ms);
    HttpConnection::ptr conn = std::make_shared<HttpConnection>(sock);
    int rt = conn->sendRequest(req);
    if(rt == 0){
        return std::make_shared<HttpResult>(static_cast<int>(HttpResult::Error::SEND_CLOSE_BY_PEER), nullptr, "send request closed by peer: " + addr->toString());
    }
    if(rt < 0){
        return std::make_shared<HttpResult>(static_cast<int>(HttpResult::Error::SEND_SOCKET_ERROR), nullptr, "send request socket error errno=" + std::to_string(errno) + " errstr=" + std::string(strerror(errno)));
    }
    auto rsp = conn->recvResponse();
    if(!rsp){
        return std::make_shared<HttpResult>(static_cast<int>(HttpResult::Error::TIMEOUT), nullptr, "recv response timeout: " + addr->toString() + " timeout_ms: " + std::to_string(timeout_ms));
    }
